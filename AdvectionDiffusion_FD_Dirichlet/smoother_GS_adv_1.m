function [ z_new ] = smoother_GS_adv_1(k,z_old,b,n1,mu)
    ...
    for l=1:n1
        for j=2:ny-1
            for i=2:nx-1
                ...
                adv = -(vel_x(i,j)*ve_sm*z_new(i+1,j)/hx-vel_x(i,j)*vw_sp*z_new(i-1,j)/hx+vel_y(i,j)*vs_sm*z_new(i,j+1)/hy-vel_y(i,j)*vn_sp*z_new(i,j-1)/hy );
                z_new(i,j) = (adv+mu*(z_new(i-1,j)/hx/hx + z_new(i+1,j)/hx/hx + z_new(i,j-1)/hy/hy + z_new(i,j+1)/hy/hy) - b(i,j))/lhs;
            end
        end
        for j=ny-1:-1:2
            for i=nx-1:-1:2
